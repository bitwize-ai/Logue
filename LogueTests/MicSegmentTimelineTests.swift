...
struct MicSegmentTimelineTests {
    @Test("A mic running the whole meeting is one placement at the start")
    func singleRunFromTheStart() {
        var timeline = MicSegmentTimeline()
        timeline.micStarted(atSessionTime: 0, fileDuration: 0)
    ...
    func lateStartIsPlacedAtItsJoinTime() {
        var timeline = MicSegmentTimeline()
        timeline.micStarted(atSessionTime: 300, fileDuration: 0)
    ...
    func mutingDoesNotShiftLaterAudio() {
        ...
        timeline.micStarted(atSessionTime: 0, fileDuration: 0)
        timeline.micStopped(fileDuration: 120)
        timeline.micStarted(atSessionTime: 420, fileDuration: 120)
        timeline.micStopped(fileDuration: 180)
        timeline.micStarted(atSessionTime: 600, fileDuration: 180)
    ...
    func trailingMuteAddsNoPlacement() {
        var timeline = MicSegmentTimeline()
        timeline.micStarted(atSessionTime: 0, fileDuration: 0)
        timeline.micStopped(fileDuration: 90)
    ...
    func emptyActivationIsDropped() {
        var timeline = MicSegmentTimeline()
        timeline.micStarted(atSessionTime: 0, fileDuration: 0)
        timeline.micStopped(fileDuration: 45)
        timeline.micStarted(atSessionTime: 300, fileDuration: 45)
    ...
    func repeatedStartIsIgnored() {
        var timeline = MicSegmentTimeline()
        timeline.micStarted(atSessionTime: 0, fileDuration: 0)
        timeline.micStarted(atSessionTime: 30, fileDuration: 30)
    ...
    func neverStartedIsEmpty() {
        let timeline = MicSegmentTimeline()
