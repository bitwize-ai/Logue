...
struct MarkdownFolderScanTests {
    ...
    // MARK: - Echo suppression in the real path

    @Test("Writes report their content to the echo filter, and reads confirm it")
    func writesAreRecognisedAsOurs() throws {
        let root = temporaryRoot()
        defer { cleanUp(root) }
        ...
        let migrator = MarkdownStorageMigrator(rootURL: root, echoFilter: filter)
        let result = migrator.export(documents: [doc.content], spaces: [])

        let url = try #require(result.writtenFiles[doc.id])
        let onDisk = try String(contentsOf: url, encoding: .utf8)
        #expect(filter.isEcho(contents: onDisk, at: url))
    }

    @Test("An outside edit to a file we wrote is not treated as ours")
    func outsideEditIsNotAnEcho() throws {
        let root = temporaryRoot()
        defer { cleanUp(root) }
        ...
        #expect(filter.isEcho(contents: edited, at: url) == false)
    }
}
